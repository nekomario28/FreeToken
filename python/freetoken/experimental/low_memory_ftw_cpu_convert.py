...
def _resolve_qwen35_nvfp4_config(model_path: str):
    """Resolve only the explicitly supported Qwen3.5-MoE NVFP4 checkpoint family."""
    from freetoken.utils import cached_load_hf_config

    hf_config = cached_load_hf_config(model_path)
    architectures = list(getattr(hf_config, "architectures", None) or [])
    if not architectures and getattr(hf_config, "text_config", None) is not None:
        architectures = list(getattr(hf_config.text_config, "architectures", None) or [])
    ...
    # Importing the model package is delayed until after the architecture check; this is an
    # execution tool, not a generic discovery path.
    ...
    if getattr(model_config, "expert_quant", None) != "nvfp4":
        raise ValueError(
            "Qwen3.5 low-memory conversion requires routed experts in native NVFP4, got "
            f"expert_quant={getattr(model_config, 'expert_quant', None)!r}"
        )
    return model_config
...
def preflight_qwen35_native_nvfp4(model_path: str, out_dir: str) -> dict[str, Any]:
    ...
    model_config = _resolve_qwen35_nvfp4_config(model_path)
