...
_MODULE_SPEC = importlib.util.spec_from_file_location(
    "freetoken_low_memory_nvfp4_under_test", _MODULE_PATH
)
...
def test_missing_global_scale_fails_closed_before_sink(tmp_path):
    config = _checkpoint(
        tmp_path, layers=1, omit="layer.0.expert.0.gate.weight_scale_2"
    )
    state = {"live_layers": 0, "max_live_layers": 0}
    calls = []
...
def test_missing_entire_layer_fails_before_any_allocation(tmp_path):
    ...
    index["weight_map"] = {
        ...
        if not name.startswith("layer.1.")
    }
